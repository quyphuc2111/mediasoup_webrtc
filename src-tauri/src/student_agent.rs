...
fn start_screen_capture(
    ...
    tokio::spawn(async move {
        println!("[ScreenCapture] Starting H.264 capture loop");
        ...
        let mut encoder = match H264Encoder::new(init_width, init_height) {
            Ok(enc) => enc,
            Err(e) => {
                println!("[ScreenCapture] Failed to create H.264 encoder: {}", e);
                return;
            }
        };
        
        println!("[ScreenCapture] H.264 encoder created for {}x{}", init_width, init_height);
            ...
            match screen_capture::capture_raw_frame() {
                Ok(raw_frame) => {
                    ...
                    match encoder.encode_rgba(&raw_frame.rgba_data, timestamp) {
                        Ok(encoded) => {
                            // Create binary frame: 
                            // [1 byte: frame_type] [8 bytes: timestamp] [4 bytes: width] [4 bytes: height] [data]
                            let mut binary_frame = Vec::with_capacity(17 + encoded.data.len());
                            ...
                            if frame_count % 30 == 0 {
                                ...
                                println!("[ScreenCapture] {} frames, {:.1} FPS, keyframe={}", 
                                    frame_count, fps, encoded.is_keyframe);
                            }
                        }
                        Err(e) => {
                            println!("[ScreenCapture] Encode error: {}", e);
                        }
                    }
                }
                Err(e) => {
