...
export function StudentAgent({ onBack }: StudentAgentProps) {
  ...
  const startAgent = async () => {
    try {
      setError(null);
      console.log(`[StudentAgent] Starting agent on port ${config.port}...`);
      ...
      console.log('[StudentAgent] WebSocket server started');
      
      // Also start the discovery listener so teacher can find us on LAN
      try {
        console.log('[StudentAgent] Starting discovery listener...');
        await invoke('start_discovery_listener', {
          name: config.student_name || 'Student',
          port: config.port,
        });
        console.log('[StudentAgent] Discovery listener started');
      } catch (discoveryError) {
        console.warn('[StudentAgent] Failed to start discovery listener:', discoveryError);
        // Don't fail the whole agent start if discovery fails
      }
    } catch (e) {
      console.error('[StudentAgent] Failed to start agent:', e);
      setError(String(e));
    }
  };
