...
use crate::crypto;
use crate::screen_capture::{self, CaptureSettings};
    ...
    loop {
        tokio::select! {
            ...
            Some(frame_data) = frame_rx.recv() => {
                ...
                // Binary frame format: 8 bytes timestamp (u64 LE) + JPEG data
                let mut binary_frame = Vec::with_capacity(8 + frame_data.len());
                binary_frame.extend_from_slice(&timestamp.to_le_bytes());
                binary_frame.extend_from_slice(&frame_data);
                
                // Send as binary WebSocket message (no base64 overhead)
                if let Err(e) = write.send(Message::Binary(binary_frame)).await {
    ...
    match msg {
        TeacherMessage::AuthResponse { signature } => {
            ...
            if result.valid {
                ...
                println!("[StudentAgent] Teacher authenticated successfully, starting auto screen share");
...
fn start_screen_capture(
    ...
    tokio::spawn(async move {
        // High quality settings for clear image
        let settings = CaptureSettings {
            quality: 85, // High quality JPEG
            target_width: 0, // Full resolution
            target_height: 0,
            scale: 1.0, // No scaling - full quality
        };
        
        println!("[ScreenCapture] Starting capture loop with quality={}", settings.quality);
        
        while !stop_flag_clone.load(Ordering::Relaxed) {
            match screen_capture::capture_frame(&settings) {
                Ok(frame_data) => {
                    // Send frame through channel
                    if frame_tx_clone.try_send(frame_data).is_err() {
        ...
        println!("[ScreenCapture] Capture loop ended");
    });
