...
pub fn capture_frame(settings: &CaptureSettings) -> Result<Vec<u8>, String> {
    ...
    let rgb_image: ImageBuffer<image::Rgb<u8>, Vec<u8>> =
        ImageBuffer::from_fn(final_image.width(), final_image.height(), |x, y| {
...
{
    println!(
        "[ScreenCapture] Starting capture loop with {}ms interval",
        interval_ms
    );
