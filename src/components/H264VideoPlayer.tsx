...
export function H264VideoPlayer({ frame, className }: H264VideoPlayerProps) {
  ...
  const decodeFrame = useCallback((frameData: ScreenFrame) => {
    ...
    if (frameData.codec === 'h264' && frameData.data_binary) {
      ...
      console.log(`[H264Player] Frame received: size=${h264Data.length}, keyframe=${frameData.is_keyframe}, has_sps_pps=${!!frameData.sps_pps}`);
      ...
      console.log(`[H264Player] Frame received (base64): size=${h264Data.length}, keyframe=${frameData.is_keyframe}`);
    }
    ...
    try {
      ...
      if (frameData.is_keyframe) {
        ...
        // Update description if available and decoder is configured
        if (frameData.sps_pps) {
          const description = arrayToUint8Array(frameData.sps_pps);
          try {
            decoder.configure({
              codec: 'avc1.42001f',
            ...
            console.log('[H264Player] Decoder reconfigured with new description');
          } catch (e) {
            console.warn('[H264Player] Failed to reconfigure with description:', e);
          }
        }
      }
      ...
      console.error('[H264Player] Decode error:', e, 'Frame size:', h264Data.length, 'isKeyframe:', frameData.is_keyframe);
