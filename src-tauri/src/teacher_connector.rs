...
pub struct ScreenFrame {
    pub data: String,  // Base64 encoded H.264 NAL units
    pub timestamp: u64,
    pub width: u32,
    pub height: u32,
            ...
            msg = read.next() => {
                match msg {
                    ...
                    Some(Ok(Message::Binary(data))) => {
                        // H.264 frame format: 
                        // [1 byte type] [8 bytes timestamp] [4 bytes width] [4 bytes height] [H.264 data]
                        if data.len() > 17 {
                            ...
                            // Store frame with base64 for frontend consumption
...
async fn handle_student_message(
    ...
    match msg {
        StudentMessage::ScreenReady { width, height } => {
            println!("[TeacherConnector] Screen ready from {} ({}x{})", id, width, height);
        ...
        StudentMessage::ScreenFrame { data, timestamp } => {
            ...
            let (width, height) = state.screen_sizes.lock()
                .ok()
