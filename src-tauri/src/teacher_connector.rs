                ...
                match msg {
                    ...
                    Some(Ok(Message::Binary(data))) => {
                        ...
                        if data.len() > 19 {
                            ...
                            // Extract AVCC description (if present)
                            ...
                            // Store frame with binary data (no base64 encoding)
                            let frame = ScreenFrame {
                                data: None,  // No base64 for H.264
                                data_binary: Some(h264_data.to_vec()),
                                sps_pps,
                                timestamp,
                                width,
                                height,
...
async fn handle_student_message(
    ...
    match msg {
        StudentMessage::ScreenReady { width, height } => {
            crate::log_debug("info", &format!("[TeacherConnector] Screen ready from {} ({}x{})", id, width, height));
