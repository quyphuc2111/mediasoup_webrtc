...
pub fn export_public_key(public_key_base64: &str) -> Result<String, String> {
    ...
    Ok(format!("-----BEGIN SMARTLAB PUBLIC KEY-----\n{}\n-----END SMARTLAB PUBLIC KEY-----", public_key_base64))
}
...
pub fn import_public_key(key_data: &str) -> Result<String, String> {
    ...
    // Validate
    let bytes = BASE64.decode(&key_data)
        .map_err(|e| format!("Invalid base64: {}", e))?;
...
pub fn sign_challenge(private_key_base64: &str, challenge: &[u8]) -> Result<String, String> {
    let private_bytes = BASE64.decode(private_key_base64)
...
pub fn verify_signature(
    ...
    let result = (|| -> Result<bool, String> {
        ...
        let key_bytes: [u8; 32] = public_bytes.try_into()
    ...
    match result {
        ...
        Err(e) => VerifyResult { valid: false, error: Some(e) },
    }
}
