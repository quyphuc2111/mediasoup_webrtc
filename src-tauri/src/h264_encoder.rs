...
impl H264Encoder {
    ...
    /// Accepts RGBA data and actual width/height (may differ from encoder dimensions)
    pub fn encode_rgba_with_size(&mut self, rgba_data: &[u8], actual_width: u32, actual_height: u32, timestamp: u64) -> Result<EncodedFrame, String> {
        // Update encoder dimensions if they changed
        if actual_width != self.width || actual_height != self.height {
            self.update_dimensions(actual_width, actual_height)?;
        }
